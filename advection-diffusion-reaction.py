...
visc = 0.01  # Estimado!
ap = 3.362*10**3
rad = math.pow(326.5*(3./(4.*math.pi)), 1./3.)
Dab = kb*293/(6*math.pi*visc*rad)
Deq = 6/ap

# Defino parámetros de diseño
eps = 1.-densb/dens
L = 4.421
Dc = 0.2
A = math.pi*(Dc/2)**2
...
uz = nu/(A)
Re = uz*Deq*densL/(visc*eps)
Pe = 0.2/eps + 0.011/eps + math.pow(eps*Re, 0.48)
Sc = visc/(densL*Dab)
Sh = 2+1.1*math.pow(Sc, 0.33)*math.pow(Re, 0.6)
Dax = Deq*uz/(eps*Pe)
kL = Sh*Dab/a
...
# Definición de un tiempo inicial para analizar y un z que descarta los últimos puntos de la resolución porque dan mal
